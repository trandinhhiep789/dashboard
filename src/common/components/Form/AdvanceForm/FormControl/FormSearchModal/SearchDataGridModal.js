...
class SearchDataGridModal extends Component {
    ...
    handleSearchSubmit(formData, MLObject) {
        const SearchParams = this.state.SearchParams.map((item, index) => {
            return {
                ...item,
                SearchValue: this.props.MLObjectDefinition[index] ? MLObject[this.props.MLObjectDefinition[index].Name] : item.SearchValue
            }
        })
        ...
        return (
            <React.Fragment>
                ...
                <div className="d-flex justify-content-end align-items-center m-1">
                    <button className="btn btn-primary mr-1" type="submit" onClick={this.handleSubmit}>Cập nhật</button>
                    <button className="btn btn-sm btn-outline btn-primary" type="button" onClick={() => this.props.hideModal()}>Bỏ qua</button>
                </div>
            </React.Fragment>
        )
    }
