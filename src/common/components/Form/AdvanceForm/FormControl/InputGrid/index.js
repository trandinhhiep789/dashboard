...
class InputGridCom extends Component {
	...
	constructor(props) {
		...
		this.state = {
			GridData: gridData, IsCheckAll: false, selected: [], PageNumber: 1,
			dataSource: this.props.dataSource, ListDataSourceMember: listDataSourceMember,
			FormValidation: {}, HasLoaded: false,
			IsSystem: isSystem
		};
	}
	...
	componentWillReceiveProps(nextProps) {
		...
		if (JSON.stringify(this.props.dataSource) !== JSON.stringify(nextProps.dataSource)) // Check if it's a new user, you can also use some unique property, like the ID
		{
			...
			if (nextProps.IDSelectColumnName) {
				if ((!this.state.GridData[nextProps.IDSelectColumnName]
					&& nextProps.dataSource && nextProps.dataSource.length > 0
					&& nextProps.IDSelectColumnName && nextProps.listColumn)
	...
	onValueChange(elementdata, index, isVavalidationError, validationErrorMessage) {
		...
		if (isVavalidationError == true || isVavalidationError == false) {
			const validationObject = { IsValidationError: isVavalidationError, ValidationErrorMessage: validationErrorMessage }
			formValidation = Object.assign({}, this.state.FormValidation, { [`${elementdata.Name}_${index}`]: validationObject });
			this.setState({
	...
	handleInsertClick() {
		...
		if (this.props.onInsertClick != null)
			this.props.onInsertClick(this.props.MLObjectDefinition, this.props.modalElementList, this.props.dataSource, formData);
	}
	...
	handleEditClick(index) {
		...
		if (this.props.onhandleEditClick != null)
			this.props.onhandleEditClick(this.props.modalMLObjectDefinition, modifyModalElementList, this.props.dataSource, formData, index);
	}
	...
	handleEditClick_BK(elementdata, dataSourceMember) {
		...
		if (this.props.onhandleEditClick != null)
			this.props.onhandleEditClick(this.props.modalMLObjectDefinition, modifyModalElementList, this.props.dataSource, formData, rowData);
	}
	handleDeleteRow(index) {
		if (this.props.onHandleDeleteRow != null)
			this.props.onHandleDeleteRow(index);
	}

	previewMedia(typeMedia, src) {
		if (this.props.previewMedia != null)
			this.props.previewMedia(typeMedia, src);
	}
	...
	renderInputGrid() {
		...
		if (this.props.IsShowRowNull)
			isShowRowNull = this.props.IsShowRowNull;
		//console.log("renderInputGrid this.props.value:", this.props.value, dataSource);
		if (this.props.value != null) {
		...
		return (
			<table className="table table-sm table-striped table-bordered table-hover table-condensed" cellSpacing="0" data-provide="datatables">
				...
				<tbody>
							...
							return (<tr key={rowIndex}>
								{
									listColumn.map((columnItem, index) => {
										...
										if (this.state.FormValidation[`${columnItem.Name}_${rowIndex}`] != null) {
											validationErrorMessage = this.state.FormValidation[`${columnItem.Name}_${rowIndex}`].ValidationErrorMessage;
										} else if (this.props.inputGridValidation !== "undefinded" && this.props.inputGridValidation != null) {
											if (this.props.inputGridValidation[`${columnItem.Name}_${rowIndex}`] != null) {
												validationErrorMessage = this.props.inputGridValidation[`${columnItem.Name}_${rowIndex}`].ValidationErrorMessage;
											}
										}
										let value;
										...
										else if (columnItem.Type == "link" || columnItem.Type == "buttonEdit" || columnItem.Type == "edit") {
											dataSourceMember = this.state.ListDataSourceMember;
											value = dataSourceMember.map((obj, index) => {
												return { key: obj.key, value: rowItem[obj.key] };
											})
					...
					{
						(dataSource == null || dataSource.length == 0) && isShowRowNull == true && <tr><td colSpan={listColumn.length} align="center">Không tồn tại dữ liệu</td></tr>
					}
				</tbody>
			</table>
		);
	...
	render() {
		...
		if (this.props.isShowFooterToolbar)
