...
class ListShipCoordinatorRouteCom extends Component {
    ...
    componentDidMount() {
        ...
        if (objRoute != undefined) {
            ...
            if (objRoute != "") {
                objInfoCoordinator = { CarrierPartnerID: objRoute.CarrierPartnerID, CarrierTypeID: objRoute.CarrierTypeID, IsRoute: true, VehicleID: objRoute.VehicleID, VehicleDriverUser: { value: objRoute.DriverUser, label: objRoute.DriverUser + "-" + objRoute.DriverUserFull } }
            }
            else {
            ...
            if (objRoute.CarrierPartnerID > 0) {
                objRoute.ShipmentOrder_DeliverUserList && objRoute.ShipmentOrder_DeliverUserList.map((item1, index) => {
                    objDeliverUser.push(item1.UserName)
                })
            }
            else {
                objRoute.ShipmentOrder_DeliverUserList && objRoute.ShipmentOrder_DeliverUserList.map((item2, index) => {
    ...
    render() {
        let { ShipmentOrder, ShipmentRouteID, ShipmentOrderSameLst, ShipmentRouteLst, ShipmentRouteSameLst,
