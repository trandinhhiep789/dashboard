...
class SearchCom extends React.Component {
    ...
    handleSearchSubmit(formData, MLObject) {
        const postData = [
            ...
            {
                SearchKey: "@BRANDID",
                SearchValue: MLObject.BrandID
            },
            {
                SearchKey: "@PRODUCTID",
                SearchValue: MLObject.ProductID ? MLObject.ProductID[0].ProductID ? MLObject.ProductID[0].ProductID : -1 : -1
            },
        ...
        this.props.callFetchAPI(APIHostName, "api/DataExportQueue/AddQueueExport", postData).then(apiResult => {
            if (!apiResult.IsError) {
                this.props.showModal(MODAL_TYPE_SHOWDOWNLOAD_EXCEL, {
                    title: "Tải file",
                    maxWidth: '1200px',
                    ParamRequest: { DataExportTemplateID: 12 }
                });
            }
            else {
                this.showMessage(apiResult.Message)
    ...
    render() {
        return (
            <React.Fragment>
                ...
                <SearchForm
                    FormName="Tìm kiếm doanh thu chi tiết"
