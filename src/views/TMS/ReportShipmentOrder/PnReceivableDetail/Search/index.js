...
class SearchCom extends React.Component {
        ...
        const postData = [
            {
                ...
                SearchValue: MLObject.ServiceGroupID
            },
            {
                SearchKey: "@RECEIVABLEDATE",
                SearchValue: toIsoStringCus(new Date(MLObject.ReceivableDate).toISOString())
            }
    ...
    handleExportSubmit(formData, MLObject) {
        ...
        const postData = {
            DataExportTemplateID: 11,
            LoadDataStoreName: 'TMS.TMS_PNRECEIVABLEDETAIL_SRH',
            KeyCached: "SHIPMENTORDER_REPORT_EXPORT",
            SearchParamList: searchData,
            ExportDataParamsDescription: "Tháng" + toIsoStringCus(new Date(MLObject.ReceivableDate).toISOString())
        }
        this.props.callFetchAPI(APIHostName, "api/DataExportQueue/AddQueueExport", postData).then(apiResult => {
            if (!apiResult.IsError) {
                this.props.showModal(MODAL_TYPE_SHOWDOWNLOAD_EXCEL, {
                    title: "Tải file",
                    maxWidth: '1200px',
                    ParamRequest: { DataExportTemplateID: 11 }
                });
            }
            else {
                this.showMessage(apiResult.Message)
    ...
    handleExportCSV(dataExport) {
        ...
        const fileName = 'Bảng kê dữ liệu doanh thu';
    ...
    render() {
        return (
            <React.Fragment>
                ...
                <SearchForm
                    FormName="Tìm kiếm bảng kê dữ liệu doanh thu"
