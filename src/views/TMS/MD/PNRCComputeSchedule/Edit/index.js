...
class EditCom extends React.Component {
    ...
    handleSubmit(formData, MLObject) {
        MLObject.PNRCComputeScheduleID = this.props.match.params.id;
        MLObject.UpdatedUser = this.props.AppInfo.LoginInfo.Username;
        MLObject.LoginLogID = JSON.parse(this.props.AppInfo.LoginInfo.TokenString).AuthenLogID;

        if (MLObject.ReceivableDate.getMonth) {
            //MLObject.ReceivableDate.setDate(MLObject.ReceivableDate.getDate() + 1);
            MLObject.ReceivableDate = toIsoStringCus(new Date(MLObject.ReceivableDate).toISOString());
        } else {
            MLObject.ReceivableDate = this.state.DataSource.ReceivableDate;
        }
    ...
    render() {
        ...
        if (this.state.IsLoadDataComplete) {
            return (
                <SimpleForm
                    FormName="Cập nhật lịch tính doanh thu phải thu từ đối tác"
