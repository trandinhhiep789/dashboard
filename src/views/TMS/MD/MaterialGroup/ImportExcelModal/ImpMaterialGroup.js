...
import {
    AddMaterialGroupByFileAPIPath,
    APIHostName,
    lstColMaterialGroupImportExcelModal,
} from '../constants';
...
class ImpMaterialGroupCom extends React.Component {
    constructor(props) {
        super(props);
    ...
    returnHeadingTitle() {
        return (
            <React.Fragment>
                <h5 className="d-flex mb-2">Danh sách nhóm vật tư<i><small className="text-danger">&nbsp;(Chỉ chấp nhận những dòng hợp lệ)</small></i></h5>
    ...
    handleSubmit() {
        ...
        this.props.callFetchAPI(APIHostName, AddMaterialGroupByFileAPIPath, uptDataSubmit).then(apiResult => {
    ...
    render() {
        return <React.Fragment>
            ...
            <DataGrid
                ...
                headingTitle={this.returnHeadingTitle("Danh sách nhóm vật tư")}
                IsAutoPaging={true}
                isHideHeaderToolbar={true}
                listColumn={lstColMaterialGroupImportExcelModal}
                PKColumnName={""}
                RowsPerPage={10}
            />
        </React.Fragment>
...
ImpMaterialGroupCom.defaultProps = {
    importData: {
        rows: [],
        errors: []
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(ImpMaterialGroupCom);