...
class AddModal extends Component {
    ...
    handleSubmit() {
        ...
        this.context.handleAddStateStaffTransferDetail(dataSubmit);
        this.props.hideModal();
    }
    ...
    handleSetDataGrid(ResultObject) {
        try {
            ...
            const { stateStaffTransferDetail } = this.context;
            const { ListCoordinatorGroup_DUser } = ResultObject;

            const notItemSelect = ListCoordinatorGroup_DUser.reduce((acc, val) => {
                const found = stateStaffTransferDetail.findIndex(item => item.FromCoordinatorGroupID == val.CoordinatorGroupID && item.UserName == val.UserName);
