...
class ModalVietBanDoShipmentRouteAuto extends Component {
  ...
  handleInit(parmLstLocation) {
    console.log("parmLstLocation", parmLstLocation);
      ...
      if (!apiResult.IsError) {
        ...
        let content = (index) => {
          let html = `<div style="height: 44px; width: 26px; position: absolute; left: 0; top: 0;">
                <i class="fa fa-map-marker fa-3x" style="position: absolute; absolute; left: 0; top: 0; z-index: 999; color: #b71540;">
        ...
        this.props.ListShipmentOrder.map((item, index) => {
          ...
          let marker = new vbd.CustomMarker({
            ...
            icon: new vbd.Icon({ size: new vbd.Size(26, 44) }),
          });

          let infoWindow = new vbd.InfoWindow({ content: templateContent(item.ShipmentOrderID == 0 ? "Kho" : `Vận đơn ${item.ShipmentOrderID}`) });
